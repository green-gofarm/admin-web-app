...
export const defaultFeedbacksPagination: PaginationProps = {
    page: 1,
...
function useFeedbacks(preventFirstCall?: boolean) {
    const dispatch = useDispatch();

    const [data, setData] = useState<any[]>([]);
    const [params, setParams] = useState(null);
    const [loading, setLoading] = useState(false);
    const [pagination, setPagination] = useState<PaginationProps>({ ...defaultFeedbacksPagination });
    ...
    const refresh = useCallback((newPagination?: PaginationProps, newParams?: any) => {
        ...
        const total: PaginationProps & any = {
            page: _pagination.page ?? defaultFeedbacksPagination.page,
            pageSize: _pagination.pageSize ?? defaultFeedbacksPagination.pageSize,
            orderBy: _pagination.orderBy ?? defaultFeedbacksPagination.orderBy,
            orderDirection: _pagination.orderDirection ?? defaultFeedbacksPagination.orderDirection,
            ..._params
        };
        ...
        dispatch(searchFeedbacks(
            { params: total },
            {
                ...
                onSuccess: (response: any) => {
                    ...
                    setPagination(pre => ({
                        ...pre,
                        totalPage: response.data?.totalPage ?? defaultFeedbacksPagination.totalPage,
                        totalItem: response.data?.totalItem ?? defaultFeedbacksPagination.totalItem,
                    }))
